...
Date (ascending) - earliest events first
Location (alphabetically) - for events on the same date
Error Handling
...
Test each endpoint with the example payloads above
Architecture
Project Structure
event-management-api/
├── config/
...
Capacity Check: Verify event capacity within transaction
Registration Insert: Add registration record
Commit/Rollback: Commit on success, rollback on failure
